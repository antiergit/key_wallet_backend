...
class BNBHelpers {
    ...
    public async getBalance(is_token: number, token_address: string, wallet_address: string) {
        try {
            ...
            if (is_token == 1) {
                balance = (await this.getBep20TokenBal(token_address, config.CONTRACT_ABI as AbiItem[], wallet_address)).toString()
            } else {
    ...
    public async getBep20TokenBal(token_address: string | undefined, tokenAbi: AbiItem[], wallet_address: string) {
        try {
            if (token_address) {
            ...
            console.error("Error in getBep20TokenBal of bnb.🔥 ~ ~", err.message)
            throw err;
        }
    }
    public async getCoinBal(address: string, inEth: boolean) {
        try {
            ...
            console.error("Error in getCoinBal of bnb  🔥 ~ ~", err.message)
            throw err;
        }
    }
    public async getTrnxDetails(transactionId: string) {
        try {
            ...
            console.error("Error in getTrnxDetails of bnb  🔥 ~ ~", err.message)
            return { status: false };
        }
    }
}

const bnbHelper = new BNBHelpers();
export default bnbHelper;
