...
class BTCTxStatusUpdateProcess {
    ...
    try {
      await utxo.getTransactionById(data.txId).then(async (rawData) => {
        if (
          ...
          rawData.confirmations >= Number(config.MIN_BLOCK_CONFORMATIONS)
        ) {
          const txUpdate = {
            status: "success",
            txid: data.txId,
  ...
  public sendTxUpdate = async (txData: { status: string; txid: string }) => {
    try {
      ...
      if (txnData) {
        let coinData: any = await btcProcessHelper.CoinByCoinId(
          txnData.coin_id
        );
        ...
        let token_type = txnData?.coin?.token_type;
        ...
        if (txUpdateRes) {
          if (status_tx == BlockChainStatusEnum.CONFIRMED) {
            ...
            let AddressWithdraw: any =
              await btcProcessHelper.check_our_wallet_address(from_address);
            if (AddressWithdraw) {
              console.log(
                "Checking btc balance getUserBtcBalance Entering getUserBtcBalance",
                from_address
              );
            ...
            let Addressdeposit: any =
              await btcProcessHelper.check_our_wallet_address(to_address);
            if (Addressdeposit) {
              ...
              console.log(
                "Checking btc balance getUserBtcBalance Addressdeposit Entering getUserBtcBalance",
                to_address
              );
            ...
            if (AddressWithdraw) {
              console.log(
                "Checking btc balance getUserBtcBalance Entering getUserBtcBalance in failed conditions",
                from_address
              );
              let balDetails: any = await utxo.getUserBtcBalance(from_address);
            ...
            if (Addressdeposit) {
              ...
              let trnxTypeD: string = "Deposit";
              console.log(
                "Checking btc balance getUserBtcBalance Addressdeposit Entering getUserBtcBalance in failed conditions",
                to_address
              );
              let balDetails: any = await utxo.getUserBtcBalance(to_address);
              /** update to adrs wallet balance */
